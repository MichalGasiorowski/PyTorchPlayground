...
def construct_conv_layers(conv_input_channels, conv_out, conv_ks, dropout, in_size=(28, 28)):
	convSequenceArray = []
...
class MyBasicNetworkBN(nn.Module):
	def __init__(self, conv_out=[24, 48], conv_ks=[3, 5], dropout=0.2, lin_out=[400, 120, 60],
		...
		self.convSequence, after_conv_output_shape = construct_conv_layers(self.conv_input_channels, self.conv_out,
		                                                                   self.conv_ks, self.dropout, self.in_size)
		...
		for i, (linear_input_size, linear_out_size) \
				in enumerate(zip(self.linear_input_sizes, self.lin_out)):
			linearSequenceArray.append(("dense"+str(i), nn.Linear(in_features=linear_input_size, out_features=linear_out_size)))
			linearSequenceArray.append(("relu"+str(i), nn.ReLU()))
			if self.dropout != 0.0 :
				convSequenceArray.append(("drop"+str(i), nn.Dropout2d(p=self.dropout)))
		self.linearSequence = nn.Sequential(OrderedDict(linearSequenceArray))
